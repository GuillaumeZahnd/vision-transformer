...
def get_alibi(nb_heads: int, nb_patches_height: int, nb_patches_width: int) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    """
    ...
    Returns:
        Linear biases, in the shape (1, nb_heads, nb_patches_height+1, nb_patches_width+1).
        Distances between patches, in the shape (nb_patches_height*nb_patches_width+1, nb_patches_height*nb_patches_width+1).
        Head-specific slope, in the shape (nb_heads).
    """

    def _get_slope(nb_heads: int) -> torch.Tensor:
        """
        ...
        def _get_slope_power_of_2(nb_heads: int) -> torch.Tensor:
            """
        ...
        if math.log2(nb_heads).is_integer():
            slope = _get_slope_power_of_2(nb_heads)

        else:
            closest_smaller_power_of_2 = 2 ** math.floor(math.log2(nb_heads))
            slope_lower = _get_slope_power_of_2(closest_smaller_power_of_2)
            slope_higher = _get_slope(2 * closest_smaller_power_of_2)[0::2][:nb_heads - closest_smaller_power_of_2]
            slope = torch.cat((slope_lower, slope_higher))
            slope, _ = torch.sort(slope, dim=0, descending=True)

        return slope

    slope = _get_slope(nb_heads=nb_heads)


    def _get_distances(nb_patches_height: int, nb_patches_width) -> torch.Tensor:
        """
        ...
        Returns:
            Distances, in the shape (nb_patches_height*nb_patches_width, nb_patches_height*nb_patches_width).
        """

        points = list(itertools.product(range(nb_patches_height), range(nb_patches_width)))
    ...
    distances = _get_distances(nb_patches_height=nb_patches_height, nb_patches_width=nb_patches_width)


    def _pad_left_and_top_for_cls_token(distances: torch.Tensor) -> torch.Tensor:
        """
            ...
            Args:
                Distance tensor, in the shape (nb_patches_height, nb_patches_width).

            Returns:
                Padded distance tensor, in the shape (nb_patches_height+1, nb_patches_width+1).
        """
        return torch.nn.functional.pad(input=distances, pad=(1, 0, 1, 0), value=0.0)

    distances = _pad_left_and_top_for_cls_token(distances=distances)
