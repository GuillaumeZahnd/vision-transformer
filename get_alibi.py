...
def get_alibi(number_of_attention_heads: int, number_of_patches_per_side: int) -> torch.Tensor:
    """
    ...
    Returns:
        Linear biases, in the shape (1, number_of_attention_heads, number_of_patches_per_side +1, number_of_patches_per_side +1).
    """

    def get_slope(number_of_attention_heads: int) -> torch.Tensor:
        """
            ...
            Returns:
                Slope, in the shape (number_of_attention_heads).
        """

        def get_slope_power_of_2(number_of_attention_heads: int) -> List[int]:
            """
            ...
            Returns:
                List of slope coefficients, of length (number_of_attention_heads).

            """

            # The starting value is effectively "2^-(8/number_of_attention_heads)", up to numerical precision.
            start = (2 ** (-2 ** -(math.log2(number_of_attention_heads) - 3)))

            ratio = start
            return [start * ratio ** i for i in range(number_of_attention_heads)]

        if math.log2(number_of_attention_heads).is_integer():
            slope = get_slope_power_of_2(number_of_attention_heads)

        else:
            closest_smaller_power_of_2 = 2 ** math.floor(math.log2(number_of_attention_heads))
            slope = \
                get_slope_power_of_2(closest_smaller_power_of_2) + \
                get_slope(2 * closest_smaller_power_of_2)[0::2][:number_of_attention_heads - closest_smaller_power_of_2]

        slope = torch.tensor(slope, dtype=torch.float32)
        slope, _ = torch.sort(slope, dim=0, descending=True)
        return slope

    slope = get_slope(number_of_attention_heads=number_of_attention_heads)
